<aside class="bg-grey-lightest p-6 pr-10 border-l border-r w-64">
    ...
    <div class="widget border-b-0">
        @if (auth()->check())
        <button class="btn is-green w-full" @click="$modal.show('new-thread')">Add New Thread</button> @else
        <button class="btn is-green w-full tracking-wide" @click="$modal.show('login')">Log In To Post</button> @endif
    </div>
    ...
    @include ('svgs.icons.all-threads', ['class' => 'mr-3 text-grey'])
            ...
            @if (auth()->check())
            <li class="pb-3">
                <a href="/threads?by={{ auth()->user()->username }}" class="flex items-center text-grey-darkest hover:text-blue hover:font-bold  {{ Request::query('by') ? 'text-blue font-bold' : '' }}">
